package main

import (
	"flag"
	"fmt"

	"github.com/starslipay/trade_id_mgr/internal/config"
	"github.com/starslipay/trade_id_mgr/internal/metrics"
	"github.com/starslipay/trade_id_mgr/internal/middleware"
	"github.com/starslipay/trade_id_mgr/internal/server"
	"github.com/starslipay/trade_id_mgr/internal/svc"
	"github.com/starslipay/trade_id_mgr/trade_id_mgr_pb"

	"github.com/zeromicro/go-zero/core/conf"
	"github.com/zeromicro/go-zero/core/service"
	"github.com/zeromicro/go-zero/zrpc"
	"google.golang.org/grpc"
	"google.golang.org/grpc/reflection"
)

var configFile = flag.String("f", "etc/tradeidmgr.yaml", "the config file")

func main() {
	flag.Parse()

	var c config.Config
	conf.MustLoad(*configFile, &c)
	ctx := svc.NewServiceContext(c)

	s := zrpc.MustNewServer(c.RpcServerConf, func(grpcServer *grpc.Server) {
		trade_id_mgr_pb.RegisterTradeIdMgrServer(grpcServer, server.NewTradeIdMgrServer(ctx))

		if c.Mode == service.DevMode || c.Mode == service.TestMode {
			reflection.Register(grpcServer)
		}
	})
	s.AddUnaryInterceptors(metrics.UnaryMetricInterceptor)
	if c.AccessLog.Enable {
		s.AddUnaryInterceptors(middleware.AccessLogUnaryInterceptor(
			middleware.NewSensitiveFields(c.AccessLog.SensitiveFields),
		))
	}
	defer s.Stop()

	fmt.Printf("Starting rpc server at %s...\n", c.ListenOn)
	s.Start()
}
